...
describe('web services', () => {
  ...
  describe('spreadsheet GET web service', () => {
    ...
    //added for prj4, not required for prj3
    it.skip('reading spreadsheet must get all current expr and values', async () => {
      const evalTestData: CellEvalTest[] = [
	[ 'c1', { expr: '42', result: { c1: 42 } } ],
	[ 'c2', { expr: 'c1', result: { c2: 42 } } ],
	[ 'c3', { expr: 'c1 + c2', result: { c3: 84 } } ],
      ...
      await setCells(SS_NAME1, evalTestData);
      await readSS(SS_NAME1, evalTestToSSDump(evalTestData, true), true);
    });
  ...
  async function readSS(ssName: string,
			dump: SSDump|[string, string, number][] ,
			doValues=false)
  {
    const url =
      doValues ? `${BASE}/${ssName}?doValues=true` : `${BASE}/${ssName}`;
    const res = await ws.get(url);
    expect(res.status === STATUS.OK);
    expect(res.body?.isOk).to.equal(true);
    const links = res.body?.links;
  ...
  function evalTestToSSDump(evalTestData: CellEvalTest[], doValues=false)
    : SSDump|[string, string, number][] 
  {
    return (doValues)
      ? evalTestData
        .map(([cellId, {expr, result}]) =>
  	    [cellId, expr.replace(/\s/g, ''), result[cellId]])
      : evalTestData
        .map(([cellId, {expr, result}]) =>
    	    [cellId, expr.replace(/\s/g, '')]);
  }
